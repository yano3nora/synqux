...
- 由来は 5 年運用された社内ブラウザゲームテンプレートの同期基盤。**仕様の正は `docs/SPEC-requests-sync.md`**（仕組み・不変条件・既知の問題・設計ガイドラインを含む）
...
- **`src/core/`**: transport 非依存の同期ステートマシン（request/response、prev チェーンによる線形化、host 判定、snapshot/restore）
- **`src/transports/`**: transport adapter 実装（当面は firebase のみ）
...
- **Documentation**:
    - 技術的な意思決定は `docs/ADR-XXXX-*.md` に記録する。大きな変更の前に既存 ADR を確認すること
    - 同期の仕組み・不変条件を変えたら `docs/SPEC-requests-sync.md` を必ず更新する
- **Versioning**: semver を厳守し CHANGELOG を保守する。消費者 repo は「テンプレは ^latest 追従、出荷済みプロジェクトは exact pin」で運用される前提

## Domains
- `request`
    - client の action を host へ判定依頼するための封筒。`requestedBy` / `prev` / `result` を持つ
- `response`
    - host による request への裁定。受理なら全端末が action を適用、拒否なら `result` の通知のみ
- `host`
    - 同期グループの単一権威。接続端末プールから決定的に導出され、離脱時は自動で移譲される (host migration)
- `revisions`
    - 処理済み request id の順序付きリスト。「実際に適用された順序」の ground truth であり、prev チェーン待機による線形化の基盤
- `snapshot`
